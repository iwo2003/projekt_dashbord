...
export async function GET(_request: Request, ctx: Ctx) {
  ...
  if (!getSiteRow(id)) return apiError("not_found", 404);
...
export async function POST(request: Request, ctx: Ctx) {
  ...
  logEvent(auth.user, parsed.data.action === "reset" ? "file.remote_reset" : "file.remote_on", { name: site.name });
  return NextResponse.json(await remoteCard(id));
}
...
export async function DELETE(_request: Request, ctx: Ctx) {
  ...
  if (!result.ok) return apiError(result.error, remoteFailureStatus(result.error));
  logEvent(auth.user, "file.remote_off", { name: site.name });
  return NextResponse.json(await remoteCard(id));
}
